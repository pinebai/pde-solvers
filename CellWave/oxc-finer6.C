...
main(int argc, char *argv[])
{
  ...
  show.setFlushFrequency(100);                                  // flush file every 10 frames
  ...
  for (int igrid=0; igrid< cg.numberOfComponentGrids(); ++igrid) {
    ...
    cconcentration[igrid](I1,I2,I3) 
      = chem.blobMin_c + ( chem.blobMax_c - chem.blobMin_c)
        * exp( -( pow( cg[igrid].vertex()(I1,I2,I3,axis1)-chem.xBlob_c,2)
	      +  pow( cg[igrid].vertex()(I1,I2,I3,axis2)-chem.yBlob_c,2))/pow( chem.blobWidth_c,2.));
  ...
  for( int istep=0; istep<=numberOfTimeSteps+1; istep++ )        // take some time steps
  {
	...
	hmax = max(hmax, max(abs(hconcentration[igrid])) );
      ...
      printf("..step %5i (t=%6.2f): Max in c=%8.3e, p=%8.3e, h=%8.3e\n", 
	     istep, tcomp, cmax, pmax, hmax);
    ...
    for( int igrid=0; igrid< cg.numberOfComponentGrids(); igrid++ ) {
      ...
      rhsArray_h = (a.d_inh - (c+ a.d_inh)* h)/a.tau_0;
