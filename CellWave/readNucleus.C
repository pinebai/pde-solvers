...
main()
{
  ...
  // initializeMappingList();   // this allows Mappings to be made with "make"
  ...
  CompositeGrid cg;
  realCompositeGridFunction u, nuclMask;
  ...
  for(;;)
  {
    ...
    if( answer=="get a solution" )
    {
      ...
      if (solutionNumber ==1) {
	std::cout << "..reading nucleus..\n"; 
	HDF_DataBase *hdf;
	hdf = showFileReader.getFrame();
	nuclMask.updateToMatchGrid(cg);
	nuclMask.get( *hdf, "nucleus mask");
      }
    ...
    else if( answer=="show nucleus mask" ) {
      psp.set(GI_TOP_LABEL,sPrintF(buff,"Nucleus mask"));
      ps.erase();
      PlotIt::contour(ps,nuclMask,psp);  
    }
