...
int main(int argc, char **argv)
{
  try {
    ...
    DPrintf(BPRINT," ------------------------------------------------------------------------- \n");
    DPrintf(BPRINT,"  .. radialSolver2Buffer ..\n");
    DPrintf(BPRINT,"      simulating Ca waves in a LiRinzel rxn w/ 2 buffers (w/ model 2Buffer)\n");
    DPrintf(BPRINT," ------------------------------------------------------------------------- \n");
    if(argc <= 1) {
      std::cerr <<"Usage: radialSolver2Buffer <parameter file name>\n";
	...
	b2Flux_prev[i] = b2Flux[i];
      ...
      printf("t=%8.3e, cmax= %8.3f, pmax= %8.3f, hmax= %8.3f, b1max= %8.3f, b2max= %8.3f\n",
