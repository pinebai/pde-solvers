...
main(int argc, char *argv[])
{
  ...
  if (argc>1) {
    ...
    std::cerr << "usage: "<<argv[0]
	      << " <grid.hdf> <output.show> <fluxBCType> <initial source grid #>"
	      << endl;
    exit(1);
  }
  if (argc>3) {
  ...
  if (argc>4) {
    initialGrid = atoi( argv[4] );
    printf("[initial grid = %d]\n", initialGrid );
  }

  // parameter values
  real t=0, dt=.002;                                     // initialize time and time step
  ...
  show.setFlushFrequency(5);                              // flush file every N frames
  ...
  int saveEvery        =1;
  int numberOfTimeSteps=110;
  //int saveEvery        =20;
  //int numberOfTimeSteps=5001;
  //int numberOfTimeSteps=50001;
...
#define ForBoundary(side,axis)   for( axis=0; axis<mg.numberOfDimensions(); axis++ ) \
	...
	ForBoundary(side,axis)   {
	  ...
	  if( mg.boundaryCondition()(side,axis) == fluxBoundaryID  ) { // flux/jump bc
	    ...
	    jump = fluxCoeff*( jump - uf(Ib1,Ib2,Ib3));
...
#undef  ForBoundary
	...
	where( mg.mask()(Ig1,Ig2,Ig3) > 0 ) {
      ...
      printf(" step %5i time= %8.3f max error = %10.4e  ... ", istep,t,  maxErr );
      printf(" t(bcs)= %10.4e, t(interior)= %10.4e, t(total step)= %10.4e\n",
	     timerBCs, timerInterior, getCPU() - timer00);
    }
    else {
      for ( int ig=0; ig< cg.numberOfComponentGrids(); ++ig ) {
	MappedGrid & mg = cg[ig];
    ...
    printf(" step %5i time= %8.3f ...", istep,t);
    printf(" t(bcs)= %10.4e, t(interp)= %10.4e, t(interior)= %10.4e, t(total step)= %10.4e\n",
	   timerBCs, timerInterpCode, timerInterior, getCPU() - timer00);
    fflush(0);
  }
  Overture::finish();          
  return 0;
