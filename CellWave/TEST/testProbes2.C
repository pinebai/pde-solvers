...
int main(int argc, char **argv)
{
  ...
  probes.openOutputFile( "$Id: testProbes2.C,v 1.1 2003/02/06 06:15:25 pfast Exp $" );

  for (int ktime=0; ktime< nSteps; ++ktime ) {
    tcomp+= dt;
    
    for( int grid=0; grid<cg.numberOfComponentGrids(); grid++ )
      {
	double scale= exp(tcomp);
	double pi = 4.*atan(1.);
	getIndex(cg[grid].indexRange(),I1,I2,I3);                
	
	u[grid](I1,I2,I3,axis1)=scale*cg[grid].vertex()(I1,I2,I3,axis1);
	u[grid](I1,I2,I3,axis2)=scale*cg[grid].vertex()(I1,I2,I3,axis2);
	if (cg.numberOfDimensions()>2) {
	  u[grid](I1,I2,I3,axis3)=scale*cg[grid].vertex()(I1,I2,I3,axis3);
	}
	else {
	  u[grid](I1,I2,I3,axis3)= scale;
	}
