...
main(int argc, char *argv[])
{
  ...
  std::string nameOfOGFile="Grids/matchingAnnuli20um.hdf", 
    nameOfShowFile="OUT/nflux2.show", 
  ...
  for( int istep=0; istep<numberOfTimeSteps; istep++ )                    // take some time steps
  {
    ...
    q.interpolate();
    q.periodicUpdate();
    Range firstComp(uComponent,uComponent);
    q.applyBoundaryCondition(uComponent,BCTypes::extrapolate,BCTypes::allBoundaries );
    if (forcingOption == ::noForcing ) { // set all physical boundaries to No-Flux
      q.applyBoundaryCondition(uComponent, BCTypes::neumann, physicalBoundaryID,  0. );
    }

    fluxBC.applyBoundaryCondition( q, uComponent );
