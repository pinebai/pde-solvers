...
main(int argc, char *argv[])
{
  ...
  std::string nameOfOGFile="Grids/sweepSweep.hdf",
    nameOfShowFile="OUT/nflux4-2.show", 
  ...
  realCompositeGridFunction rhsp(cg,all,all,all,1);
  ...
  if ( forcingOption >0 ) {
    ...
    for ( int ig=0; ig< cg.numberOfComponentGrids(); ++ig ) {
      ...
      u[ig](I1,I2,I3) = exact( mg, I1,I2,I3, 0, 0.) + 3;
  ...
  int saveEvery        =10,  logEvery=20;
  //int saveEvery        =1,  logEvery=1;
  ...
  for( int istep=0; istep<numberOfTimeSteps; istep++ )                    // take some time steps
  {
    ...
    for ( int ig=0; ig< cg.numberOfComponentGrids(); ++ig ) {
      ...
      q[ig](I1,I2,I3, uRHSComponent)     +=  reactionFactor*q[ig](I1,I2,I3,uComponent)*(1- q[ig](I1,I2,I3,uComponent));
      if (forcingOption > 0) {
	OGFunction & exact = *pExactSolution; // make a reference for readibility
	...
	q[ig](I1,I2,I3, uForcingComponent) =  ( exact.t( mg, I1,I2,I3, 0, t) 
						-viscosity*exact.laplacian(mg, I1,I2,I3,0,t));
	q[ig](I1,I2,I3, uRHSComponent)     -=  reactionFactor*exact(mg,I1,I2,I3,0,t)*(1- exact(mg,I1,I2,I3,0,t));
	...
	rhsp[ig](I1,I2,I3) = q[ig](I1,I2,I3,uRHSComponent); 
      }
      else {
	if (ig==0 && logThisStep ) printf("..ab2..");
      }
      unext[ig](I1,I2,I3) = u[ig](I1,I2,I3) + dt*( (3./2.)*q[ig](I1,I2,I3, uRHSComponent) 
						   -(1./2.)  *rhsp[ig](I1,I2,I3 ) );
      rhsp[ig](I1,I2,I3) = q[ig](I1,I2,I3, uRHSComponent );
    } //end for ig
    ...
    u.applyBoundaryCondition(0,BCTypes::extrapolate, physicalBoundaryID,0.);    //extrap. ghostlines in u
    u.applyBoundaryCondition(0,BCTypes::extrapolate,     fluxBoundaryID,0.);    
    if (forcingOption == ::noForcing ) { // set all physical boundaries to No-Flux
      ...
      u.applyBoundaryCondition(0,BCTypes::neumann,   fluxBoundaryID,0.);    
