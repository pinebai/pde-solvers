...
//  show.setFlushFrequency(10);                                  // flush file every 10 frames
  ...
  HDF_DataBase *hdfFrame;    
  show.startFrame();                             
  hdfFrame = show.getFrame(); assert( hdfFrame != NULL);
  ...
  realCompositeGridFunction nuc(cg,all,all,all,1);               // create a grid function 
  Index I1,I2,I3;
  for( int ig=0; ig< cg.numberOfComponentGrids();  ++ig ) {
    getIndex(cg[ ig ].indexRange(),I1,I2,I3);
    const double twopi=2.*4.*atan(1.); 
    nuc[ig](I1,I2,I3) = sin(twopi*cg[ ig ].vertex()(I1,I2,I3,axis1)/500.) 
                        *cos(twopi*cg[ ig ].vertex()(I1,I2,I3,axis2)/500.);  
  }
  nuc.put( *hdfFrame, "nucleus mask");
  ...
  {
    if( i % 40 == 0 )  // save solution every 10 steps
    {
      if (i !=0 ) show.startFrame();                                         // start a new frame
