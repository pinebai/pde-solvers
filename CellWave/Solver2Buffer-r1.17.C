...
solve()
{
  ...
  for( ktime=0; ktime<=data.numberOfTimeSteps+1; ++ktime )
  {
	...
	doneLookingForTop:
	  if( foundTheTop ) {
	    DPrintf(DebugSolver, "   IP3 influx surf found: grid %d, side %d, axis %d\n",
		    igrid, side, axis); fflush(0);
...
#define YC xArray(Ib1,Ib2,Ib3,axis2)
	    ...
	    double fluxMax= max(abs(ip3Flux));
	    const int thisBCIndex=BCTypes::boundary1+side+2*axis;
	    DPrintf(DebugSolver, "    setting IP3 influx: grid %d, fluxMax=%8.4e\n",
		    igrid, fluxMax);
