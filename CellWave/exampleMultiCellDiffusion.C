...
//  show.setFlushFrequency(10);                                  // flush file every 10 frames
  ...
  for( int ig=0; ig<cg.numberOfGrids(); ++ig) {
    ...
    getIndex(cg[ig].dimension(),I1,I2,I3); // assign all points including ghost points.
    cg[ig].update(MappedGrid::THEvertex);  // build the array of vertices
    //const realArray & vertex = cg[ig].vertex();
    if( ig ==0 ) {
      u[ig]=1.;
    } else {
      u[ig]=0.;
    }
    //  where( vertex(I1,I2,I3,1) > 0. ) {
    //  u[ig]=1.;
    //};
  }

  real t=0, dt=.02;                                           // initialize time and time step
  real a=0., b=0., viscosity=1.;                               // initialize parameters
    
  char buffer[80];                                             // buffer for sprintf
  int numberOfTimeSteps=5000;
  for( int i=0; i<numberOfTimeSteps; i++ )                    // take some time steps
  {
    if( i % 50 == 0 )  // save solution every 10 steps
    {
    ...
    u.applyBoundaryCondition(0,BCTypes::neumann,BCTypes::allBoundaries,0.);
